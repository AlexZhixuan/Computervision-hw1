...
def create_color_to_emoji_mapping(quantized_colors):
    ...
    base_path = "/Users/nacl/Desktop/noto-emoji/png/128"
    color_emoji_map = {}
...
def create_artistic_tile(color, size, style="3D Effect", color_emoji_map=None):
    ...
    if style == "3D Effect":
        ...
        # Try to load consistent emoji for this color
        if color_emoji_map:
            emoji_tile = load_local_emoji(color, (w, h), color_emoji_map)
            if emoji_tile:
                tile = emoji_tile
            else:
                ...
                if radius > 2:
                    ...
                    draw.ellipse([center_x-radius, center_y-radius, center_x+radius, center_y+radius], fill=highlight)
